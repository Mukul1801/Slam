...
public class HitPointManager : MonoBehaviour
{
    ...
    private void UpdatePathCreationMode()
    {
        ...
        if (width < Screen.width || height < Screen.height)
        {
            ...
            bool hasDetectedHit = raycastManager.Raycast(new Vector2(width, height), raycastHitList, TrackableType.FeaturePoint | TrackableType.PlaneWithinPolygon);
    ...
    private void CreateWaypointVisual(int poseIndex)
    {
        ...
        switch (pose.waypointType)
        {
            ...
            case WaypointType.Obstacle:
                waypointObject = Instantiate(obstaclePrefab, pose.position, pose.rotation);
                waypointObject.tag = "Obstacle";
                break;
